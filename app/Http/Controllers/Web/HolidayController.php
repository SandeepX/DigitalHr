...
class HolidayController extends Controller
{
    ...
    public function store(HolidayRequest $request): RedirectResponse
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->back()->with('success', 'New Holiday Detail Added Successfully');
        } catch (Exception $e) {
            DB::rollBack();
    ...
    public function update(HolidayRequest $request, $id): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Holiday Detail Updated Successfully');
        } catch (Exception $exception) {
            return redirect()->back()->with('danger', $exception->getMessage())
                ->withInput();
        }
    ...
    public function importHolidays(Request $request)
    {
        ...
        try {
            ...
            if ($countheader < 4 && in_array('event', $header) && in_array('event_date', $header) && in_array('note', $header)) {
                ...
                return redirect()->back()->with('success', 'Holidays Detail Imported Successfully');
            } else {
                return redirect()->back()->with('danger', 'Your CSV files having unmatched Columns to our database. Your columns must be in this sequence  event, event_date, note  only');
            }
        } catch (Exception $exception) {
            return redirect()->back()->with('danger', $exception->getMessage());
        }
